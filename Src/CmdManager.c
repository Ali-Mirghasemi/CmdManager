...
void CmdManager_handle(CmdManager* manager, IStream* stream, char* buffer, Str_LenType len) {
    if (IStream_available(stream) > 0) {
        ...
        if (lineLen > 0) {
            ...
            // remove endWith
            pBuff[lineLen - manager->EndWith->Len] = '\0';
            // check start with
            ...
            // find cmd
            cmdIndex = __search(manager->List.Cmds, manager->List.Len, sizeof(manager->List.Cmds[0]), pBuff, Cmd_compareName);
            if (cmdIndex != -1) {
            #if CMD_LIST_MODE == CMD_LIST_ARRAY
                Cmd* cmd = &manager->List.Cmds[cmdIndex];
                pBuff += manager->List.Cmds[cmdIndex].CmdName.Len;
            #else
                Cmd* cmd = manager->List.Cmds[cmdIndex];
                pBuff += manager->List.Cmds[cmdIndex]->CmdName.Len;
            #endif // CMD_LIST_MODE
                if (manager->PatternTypes) {
                    // find cmd type
                    Mem_LenType typeIndex = Mem_linearSearch(manager->PatternTypes->Patterns, CMD_TYPE_LEN, sizeof(Cmd_Str*), pBuff, CmdType_compare);
                    if (typeIndex != -1) {
                        manager->Params.Cursor = pBuff;
                        if (cmd->Callbacks.fn[typeIndex]) {
                            cmd->Callbacks.fn[typeIndex](cmd, &manager->Params, (Cmd_Type) typeIndex);
                            return;
                        }
                        else if (cmd->Callbacks.unknown) {
                            cmd->Callbacks.unknown(cmd, &manager->Params, (Cmd_Type) typeIndex);
                            return;
                        }
                    }
                }
...
static char Cmd_compareName(const void* name, const void* cmd, Mem_LenType itemLen) {
#if CMD_LIST_MODE == CMD_LIST_ARRAY
    return Str_compareFix((char*) name, ((Cmd*) cmd)->CmdName.Name, ((Cmd*) cmd)->CmdName.Len);
#else
    return Str_compareFix((char*) name, (*(Cmd**) cmd)->CmdName.Name, (*(Cmd**) cmd)->CmdName.Len);
#endif // CMD_LIST_MODE
}
...
#if CMD_LIST_MODE == CMD_LIST_ARRAY
    Cmd temp;
    Mem_copy((uint8_t*) &temp, itemA, sizeof(Cmd));
    Mem_copy(itemA, itemB, sizeof(Cmd));
    Mem_copy(itemB, (uint8_t*) &temp, sizeof(Cmd));
#else
    Cmd* temp;
    temp = itemA;
    itemA = itemB;
...
static char CmdType_compare(const void* buff, const void* type, Mem_LenType itemLen) {
    printf("[Debug] buff: %s\n", (char*) buff);
    printf("[Debug] type: {%s, %d}\n", (*(Cmd_Str**) type)->Name, (*(Cmd_Str**) type)->Len);

    if ((*(Cmd_Str**) type)->Len > 0) {
        return Str_compareFix((char*) buff, (*(Cmd_Str**) type)->Name, (*(Cmd_Str**) type)->Len);
    }
    else {
        return *(char*) buff != *(*(Cmd_Str**) type)->Name;
    }
}
